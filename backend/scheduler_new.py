...
def get_bcm_plugin():
    try:
        import backend.bcm.plugin as bcm_plugin
        return bcm_plugin
    except ImportError:
        return None
...
def start_bcm_session_scheduler_loop():
    plugin = get_bcm_plugin()
    if plugin and hasattr(plugin, 'start_bcm_session_scheduler_loop'):
        plugin.start_bcm_session_scheduler_loop(scheduler, _job_bcm_session_scheduler, _register_scheduled_session)
    else:
