      ...
      engine.layout.tiles.forEach((tile, index) => {
        ...
        if (tile === 0) {
          ...
          if (floorImage.complete) {
            ...
            ctx.fillStyle = tile === 4 ? 'rgba(180,110,50,.14)' : tile === 7 ? 'rgba(16,41,48,.42)' : tile === 2 ? 'rgba(31,38,54,.38)' : 'rgba(24,31,44,.42)';
            ctx.fillRect(x, y, OFFICE_TILE_SIZE * scale, OFFICE_TILE_SIZE * scale);
          }
          else { ctx.fillStyle = tile === 4 ? '#4a382a' : tile === 7 ? '#263d46' : '#303948'; ctx.fillRect(x, y, OFFICE_TILE_SIZE * scale, OFFICE_TILE_SIZE * scale); }
        }
      });
