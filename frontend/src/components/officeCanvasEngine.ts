...
export function createDefaultCanvasLayout(): CanvasOfficeLayout {
  ...
  const tiles = Array.from({ length: cols * rows }, (_, index) => {
    ...
    if (col >= 18) return 4; // Distinct cozy lounge floor for the entire Lounge Zone
    if (row >= 11) return 7;
    return 1;
  });
  ...
  // Wall Decor & Bookshelves
  place('DOUBLE_BOOKSHELF', 3, 1); place('DOUBLE_BOOKSHELF', 7, 1); place('CLOCK', 11, 1); place('WHITEBOARD', 14, 1);

  // Work Stations (Left Office Zone)
  [[2, 3], [7, 3], [12, 3], [2, 7], [7, 7], [12, 7], [2, 13], [7, 13], [12, 13], [2, 17], [7, 17], [12, 17]].forEach(([col, row]) => {
    place('DESK_FRONT', col, row); place('PC_FRONT_OFF', col + 1, row); place('CUSHIONED_CHAIR_BACK', col + 1, row + 2);
  });

  // Plants & Room Dividers
  place('PLANT', 1, 3); place('PLANT', 16, 3); place('PLANT', 16, 9); place('PLANT', 1, 17); place('PLANT', 16, 17);

  // --- REAL LOUNGE ZONE (Right Side, col >= 18) ---
  // Coffee & Beverage Bar (Upper Lounge)
  place('DOUBLE_BOOKSHELF', 19, 1); place('SMALL_PAINTING', 22, 1); place('CLOCK', 25, 1);
  place('COFFEE_TABLE', 20, 3); place('CUSHIONED_BENCH', 19, 3); place('CUSHIONED_BENCH', 22, 3); place('LARGE_PLANT', 25, 2);

  // Main Sofa Lounge (Middle Lounge)
  place('SOFA_FRONT', 20, 6); place('SOFA_FRONT', 24, 6);
  place('COFFEE_TABLE', 22, 8); place('CUSHIONED_BENCH', 20, 8); place('CUSHIONED_BENCH', 24, 8);
  place('PLANT', 19, 6); place('PLANT', 26, 6);

  // Social & Relaxation Lounge (Lower Lounge)
  place('WHITEBOARD', 21, 12); place('LARGE_PLANT', 19, 12); place('LARGE_PLANT', 26, 12);
  place('SOFA_FRONT', 20, 15); place('SOFA_FRONT', 24, 15);
  place('COFFEE_TABLE', 22, 17);
  place('CUSHIONED_BENCH', 19, 17); place('CUSHIONED_BENCH', 20, 17); place('CUSHIONED_BENCH', 24, 17); place('CUSHIONED_BENCH', 25, 17);
...
export class CanvasOfficeEngine {
  ...
  setAgents(agents: CanvasAgentData[]) {
    ...
    agents.forEach(agent => {
      ...
      this.characters.set(agent.id, {
        ...
        direction: 'up', state: 'idle', path: [], frame: 0, frameTimer: 0,
        palette: hashString(agent.id) % 6, seat: null, wanderTimer: 50 + (hashString(agent.id) % 30), spawnTimer: .7,
  ...
  private seats() {
    ...
    this.layout.furniture.forEach(item => {
      ...
      if (spec.seat === 'work') {
        seats.push({ col: item.col, row: item.row, purpose: 'work' });
      } else if (spec.seat === 'lounge') {
        seats.push({ col: item.col, row: item.row, purpose: 'lounge' });
      }
      if (item.kind === 'SOFA_FRONT') {
        for (let offset = 0; offset < spec.footprintW; offset += 1) {
          seats.push({ col: item.col + offset, row: item.row + 1, purpose: 'lounge' });
        }
      }
    });
    return seats;
  ...
  private sendToSeat(character: CanvasCharacter) {
    ...
    if (character.path.length) {
      ...
      character.state = (character.seat && character.seat.purpose === 'lounge') || this.wantsLounge(character) ? 'rest' : 'idle';
    }
  }
    ...
    this.characters.forEach(character => {
      ...
      if (!character.path.length) {
        ...
        if (character.wanderTimer <= 0) {
          const isWorking = character.agent.statusKind === 'working';
          const roll = hashString(character.agent.id + String(Math.floor(this.clock))) % 100;
          if (isWorking && roll < 70) {
            this.sendToSeat(character);
          } else {
            this.randomWalkable(character);
          }
          character.wanderTimer = 50 + (hashString(character.agent.id + String(Math.floor(this.clock * 10))) % 30);
        }
      }
    });
