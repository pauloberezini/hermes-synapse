...
export function IsoOfficeCanvas({
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      const roomCharacters = [...engine.characters.values()]
        .filter(character => roomForCharacter(character) === room.id)
        .sort((a, b) => a.agent.id.localeCompare(b.agent.id));

      roomCharacters.forEach((character, index) => {
        const spotInfo = getRoomCharacterScenePoint(character, room, index);
        const point = spotInfo.point;
        const charDirection = spotInfo.direction;
        const spotEffect = spotInfo.effect;
        const charState = spotInfo.state;
        ...
        drawables.push({
          ...
          draw: () => {
            ...
            const bob = moving ? Math.abs(Math.sin(phase)) * 2.2 * unit : 0;
            const lean = moving ? Math.sin(phase) * .018 : 0;
            const restScale = charState === 'rest' ? .96 : 1;
            const image = imageFor(imagesRef.current, isoCharacterAsset(character.agent, charDirection, character.specialist));
