import type { CanvasAgentData, CanvasCharacter, CanvasCharacterState, CanvasDirection } from './officeCanvasEngine';
...
export const ISO_ROOMS: IsoRoomScene[] = [
  ...
  },
  {
    ...
    assets: [
      ...
      { id: 'lounge-filing', src: furniture('filling-open'), x: 65.5, y: 56.5, height: 60 },
      { id: 'lounge-board', src: culture('todo-board'), x: 42.0, y: 42.0, height: 66, motion: 'signal' },
...
export interface IsoCharacterSpot {
  x: number;
  y: number;
...
export const ROOM_ACTIVITY_SPOTS: Record<IsoRoomId, IsoCharacterSpot[]> = {
  lounge: [
    { x: 27, y: 62, direction: 'left' },                             // Looking out the panoramic window
    { x: 74, y: 56, direction: 'up', effect: 'need-coffee' },         // Brewing coffee at espresso machine
    { x: 33, y: 68, direction: 'left', effect: 'star' },              // Daydreaming by the window
    { x: 62, y: 48, direction: 'up', effect: 'glass-water' },         // Drinking water at the cooler
    { x: 48, y: 62, direction: 'down', state: 'rest' },               // Relaxing on sofa spot 1
    { x: 55, y: 67, direction: 'right', state: 'rest' },              // Relaxing on sofa spot 2
    { x: 84, y: 68, direction: 'down', effect: 'post-it' },           // Reading near plant corner
    { x: 44, y: 48, direction: 'up' },                                // Reviewing team Todo board
    { x: 36, y: 74, direction: 'right' },                             // Plant relaxation nook
    { x: 64, y: 60, direction: 'down', state: 'rest' },               // Resting in lounge area
    { x: 42, y: 56, direction: 'right', effect: 'monster-energy' },   // Energy drink break
    { x: 70, y: 46, direction: 'down' },                              // Breakroom entrance spot
  ],
  studio: [
    { x: 46, y: 55, direction: 'up', state: 'type' },
    { x: 41, y: 52, direction: 'up', state: 'type' },
    { x: 39, y: 57, direction: 'left', state: 'type' },
    { x: 35, y: 69, direction: 'up', state: 'type' },
    { x: 31, y: 66, direction: 'up', state: 'type' },
    { x: 85, y: 59, direction: 'up', effect: 'typing' },
    { x: 45, y: 59, direction: 'down' },
    { x: 32, y: 73, direction: 'right' },
  ],
  research: [
    { x: 47, y: 58, direction: 'up', state: 'type' },
    { x: 39, y: 54, direction: 'up', state: 'type' },
    { x: 32, y: 62, direction: 'left', state: 'type' },
    { x: 52, y: 70, direction: 'up', state: 'type' },
    { x: 43, y: 75, direction: 'right', state: 'type' },
    { x: 78, y: 45, direction: 'up', effect: 'post-it' },
    { x: 82, y: 61, direction: 'up' },
  ],
  meeting: [
    { x: 43, y: 61, direction: 'up', state: 'type' },
    { x: 51, y: 65, direction: 'left', state: 'type' },
    { x: 36, y: 65, direction: 'right', state: 'type' },
    { x: 79, y: 44, direction: 'up', effect: 'post-it' },
    { x: 63, y: 42, direction: 'up', effect: 'thumb-up' },
    { x: 30, y: 64, direction: 'down' },
  ],
  control: [
    { x: 49, y: 64, direction: 'up', state: 'type' },
    { x: 40, y: 59, direction: 'up', state: 'type' },
    { x: 32, y: 68, direction: 'left', state: 'type' },
    { x: 88, y: 45, direction: 'up', effect: 'build-failed' },
    { x: 80, y: 60, direction: 'up' },
  ],
};

export function getRoomCharacterScenePoint(
  character: CanvasCharacter,
  room: IsoRoomScene,
  index: number
): { point: IsoPoint; direction: CanvasDirection; effect?: string; state?: CanvasCharacterState } {
  if (character.state === 'walk' || character.path.length > 0) {
    return {
      point: projectCharacterToScene(character, room, 28, 22),
      direction: character.direction,
      effect: undefined,
      state: 'walk',
    };
  }
  const spots = ROOM_ACTIVITY_SPOTS[room.id];
  if (spots && spots.length > 0) {
    const spotIndex = index % spots.length;
    const cycle = Math.floor(index / spots.length);
    const spot = spots[spotIndex];
    const offsetX = cycle > 0 ? ((cycle * 5) % 15) - 7 : 0;
    const offsetY = cycle > 0 ? ((cycle * 4) % 12) - 6 : 0;
    return {
      point: { x: spot.x + offsetX, y: spot.y + offsetY },
      direction: spot.direction || character.direction,
      effect: spot.effect,
      state: spot.state || character.state,
    };
  }
  return {
    point: projectCharacterToScene(character, room, 28, 22),
